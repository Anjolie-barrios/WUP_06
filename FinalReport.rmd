...
White and Other have the smallest differences in beliefs; 1.2482 is the odds ratiot there.  
```{r}
fisherXY(GSScsv$RACE,GSScsv$EVOLVED,confL=.95,printAll = T)
```
  
With p-values of 1, 0.3586, and 0.3171, comparisons between EVOLVED and race are bound to support a null hypothesis: that race and EVOLVED have little to no true correlation.   
Black vs Other and White vs Other have odds ratios that are close to each other (slightly more than 0.5), which makes sense given that the graphs show that those of Other races have more of a tendency to disbelieve in evolution.  
```{r}
